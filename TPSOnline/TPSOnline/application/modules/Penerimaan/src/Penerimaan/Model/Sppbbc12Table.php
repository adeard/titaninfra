...
class Sppbbc12Table extends AbstractTableGateway
{
    protected $table;
	protected $adapter;
	public $connection;
    ...
    public function __construct(Adapter $adapter)
    {
		...
		$this->table 		= new TableIdentifier('SPPBBC12', SCHEMA_TPS);
		$this->initialize();
    }
	...
	public function fetchAll($param)
    {
		...
		foreach($data as $entry) {
			...
			*/
			$action	= '	<a data-href="'.BASEPATH.'/penerimaan/sppbpib/detail/'.$entry['ID_SPPBBC12'].'" rel="tooltip" title="Detail" class="ajaxify">
							<button class="btn btn-primary btn-sm"><i class="fa fa-search"></i></button>
						</a>';
	   		
			$row[]	= array('<input type="checkbox" name="UID[]" id="UID" value="'.$entry['ID_SPPBBC12'].'" class="checkboxes">',
							$entry['ID_SPPBBC12'],
							$entry['CAR'],
							$entry['NO_SPPB'],
							$entry['TGL_SPPB'],
    ...
    public function getData($data)
	{
		...
		$where 		= new  Where();
        $where->equalTo('ID_SPPBBC12', $data['ID_SPPBBC12']);
        $select->where($where);
	...
	public function save($data)
    {
        $aColumns = array(	'IDPERUSAHAAN'		=> $data['IDPERUSAHAAN'],
							...
							'TG_BL_AWB' 		=> $data['TG_BL_AWB'],
							'NO_MASTER_BL_AWB' 	=> $data['NO_MASTER_BL_AWB'],
							'TG_MASTER_BL_AWB' 	=> $data['TG_MASTER_BL_AWB'],
						  	'CRBY' 				=> $data['CRBY'],
	...
	public function edit($data)
	{
		...
		try {
			...
			$sQuery		= $this->sql->update($this->table)
									->set($aColumns)
									->where(array('ID_SPPBBC12' => $data['ID_SPPBBC12']));
