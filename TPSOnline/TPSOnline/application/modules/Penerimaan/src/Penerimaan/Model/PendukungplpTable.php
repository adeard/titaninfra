...
class PendukungplpTable extends AbstractTableGateway
{
    protected $table;
	protected $adapter;
	public $connection;
    ...
    public function __construct(Adapter $adapter)
    {
		...
		$this->table 		= new TableIdentifier('PENDUKUNGPLP', SCHEMA_TPS);
		$this->initialize();
    }
	...
	public function fetchAll($param)
    {
		...
		$aColumns 	= array('a.ID_PENDUKUNGPLP',
							'a.ID_PENDUKUNGPLP', 
							'a.NO_BC11', 
							'a.TGL_BC11', 
							'a.NO_CONT', 
		...
		if($param['NO_BC11'] != ''):
			$select->where(array('"a"."NO_BC11" = '."'".$param['NO_BC11']."'".' '),
						   \Zend\Db\Sql\Predicate\PredicateSet::OP_AND);
			$selrows->where(array('"a"."NO_BC11" = '."'".$param['NO_BC11']."'".' '),
							\Zend\Db\Sql\Predicate\PredicateSet::OP_AND);
		endif;
		
		if($param['TGL_BC11'] != ''):
			$select->where(array('"a"."TGL_BC11" = '."'".$param['TGL_BC11']."'".' '), 
						   \Zend\Db\Sql\Predicate\PredicateSet::OP_AND);
			$selrows->where(array('"a"."TGL_BC11" = '."'".$param['TGL_BC11']."'".' '), 
							\Zend\Db\Sql\Predicate\PredicateSet::OP_AND);
		endif;
		
		if($param['NO_CONT'] != ''):
			$select->where(array('"a"."NO_CONT" = '."'".$param['NO_CONT']."'".' '),
						   \Zend\Db\Sql\Predicate\PredicateSet::OP_AND);
			$selrows->where(array('"a"."NO_CONT" = '."'".$param['NO_CONT']."'".' '),
							\Zend\Db\Sql\Predicate\PredicateSet::OP_AND);
		endif;
			
		
		$select->from(array('a' => $this->table))
			   ->columns(array( 'ID_PENDUKUNGPLP', 
								'NO_BC11', 
								'TGL_BC11', 
								'NO_CONT', 
								'NO_SEGEL',
		...
		foreach($data as $entry) {
			...
			*/
			$action	= '	<a data-href="'.BASEPATH.'/penerimaan/pendukungplp/detail/'.$entry['ID_PENDUKUNGPLP'].'" rel="tooltip" title="Detail" class="ajaxify">
							<button class="btn btn-primary btn-sm"><i class="fa fa-search"></i></button>
						</a>';
	   		
			$row[]	= array('<input type="checkbox" name="UID[]" id="UID" value="'.$entry['ID_PENDUKUNGPLP'].'" class="checkboxes">',
							$entry['ID_PENDUKUNGPLP'],
							$entry['NO_BC11'],
							$entry['TGL_BC11'],
							$entry['NO_CONT'],
							$entry['NO_SEGEL'],
    ...
    public function getData($data)
	{
		...
		$where 		= new  Where();
        $where->equalTo('ID_PENDUKUNGPLP', $data['ID_PENDUKUNGPLP']);
        $select->where($where);
	...
	public function edit($data)
	{
		...
		try {
			...
			$sQuery		= $this->sql->update($this->table)
									->set($aColumns)
									->where(array('ID_PENDUKUNGPLP' => $data['ID_PENDUKUNGPLP']));
    ...
    public function delete($id)
    {
		...
		try 
		{
			...
			$where 		= new  Where();
			$where->equalTo('ID_PENDUKUNGPLP', $id);
			$delete->where($where);
