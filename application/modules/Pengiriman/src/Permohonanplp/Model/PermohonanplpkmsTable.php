...
class PermohonanplpkmsTable extends AbstractTableGateway
{
    protected $table;
	protected $adapter;
	public $connection;
	...
	public function fetchAll($param)
    {
		...
		foreach($data as $entry) {
			...
			*/
			$action	= '	<a data-href="'.BASEPATH.'/pengiriman/permohonanplpkms/edit/'.$entry['ID_PLP'].'" rel="tooltip" title="Edit" class="ajaxify">
							<button class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></button>
						</a> 
						
                       	<a data-href="'.BASEPATH.'/pengiriman/permohonanplpkms/delete/'.$entry['ID_PLP'].'" rel="tooltip" title="Delete" class="delete">
							<button class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></button> 
						</a>';
	   		
			$row[]	= array('<input type="checkbox" name="UID[]" id="UID" value="'.$entry['ID_PLP'].'" class="checkboxes">',
							$entry['ID_PLP'],
							'<a data-href="'.BASEPATH.'/pengiriman/permohonanplpkms/edit/'.$entry['ID_PLP'].'" rel="tooltip" title="Edit" class="ajaxify">'.$entry['KD_KANTOR'].'</a>',
							$entry['TIPE_DATA'],
							$entry['KD_TPS_ASAL'],
							$entry['REF_NUMBER'],
							$entry['NO_SURAT'],
	...
	{
		$aColumns = array(	'ID_SERVICE_TYPE' 	=> 122,
						  	'KD_KANTOR'			=> $data['KD_KANTOR'],
						  	'TIPE_DATA'			=> $data['TIPE_DATA'],
							'KD_TPS_ASAL'		=> $data['KD_TPS_ASAL'],
							'REF_NUMBER'		=> $data['REF_NUMBER'],
