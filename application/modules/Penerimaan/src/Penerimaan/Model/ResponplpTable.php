...
class ResponplpTable extends AbstractTableGateway
{
    protected $table;
	protected $adapter;
	public $connection;
	...
	public function fetchAll($param)
    {
		...
		$select->where(array('"a"."ISTERIMA" = '.$param['ISTERIMA'].' '),
					   \Zend\Db\Sql\Predicate\PredicateSet::OP_AND);
		$selrows->where(array('"a"."ISTERIMA" = '.$param['ISTERIMA'].' '),
						\Zend\Db\Sql\Predicate\PredicateSet::OP_AND);
		...
		foreach($data as $entry) {
			...
			*/
			$action	= '	<a data-href="'.BASEPATH.'/penerimaan/responplp/detail/'.$entry['ID_RESPONPLP'].'" rel="tooltip" title="Detail" class="ajaxify">
