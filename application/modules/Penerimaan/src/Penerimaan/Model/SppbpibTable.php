...
class SppbpibTable extends AbstractTableGateway
{
    protected $table;
	protected $adapter;
	public $connection;
    ...
    public function __construct(Adapter $adapter)
    {
		...
		$this->table 		= new TableIdentifier('SPPBPIB', SCHEMA_TPS);
		$this->initialize();
    }
	...
	public function fetchAll($param)
    {
		...
		foreach($data as $entry) {
			...
			*/
			$action	= '	<a data-href="'.BASEPATH.'/penerimaan/sppbpib/detail/'.$entry['ID_SPPBPIB'].'" rel="tooltip" title="Detail" class="ajaxify">
							<button class="btn btn-primary btn-sm"><i class="fa fa-search"></i></button>
						</a>';
	   		
			$row[]	= array('<input type="checkbox" name="UID[]" id="UID" value="'.$entry['ID_SPPBPIB'].'" class="checkboxes">',
							$entry['ID_SPPBPIB'],
							$entry['CAR'],
							$entry['NO_SPPB'],
							$entry['TGL_SPPB'],
    ...
    public function getData($data)
	{
		...
		$where 		= new  Where();
        $where->equalTo('ID_SPPBPIB', $data['ID_SPPBPIB']);
        $select->where($where);
	...
	public function edit($data)
	{
		...
		try {
			...
			$sQuery		= $this->sql->update($this->table)
									->set($aColumns)
									->where(array('ID_SPPBPIB' => $data['ID_SPPBPIB']));
