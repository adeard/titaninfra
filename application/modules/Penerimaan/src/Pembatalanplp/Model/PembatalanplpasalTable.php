...
class PembatalanplpasalTable extends AbstractTableGateway
{
    protected $table;
	protected $adapter;
	public $connection;
	...
	public function fetchAll($param)
    {
		...
		foreach($data as $entry) {
			...
			*/
			$action	= '	<a data-href="'.BASEPATH.'/penerimaan/pembatalanplpasal/edit/'.$entry['ID_RESPONPLP'].'" rel="tooltip" title="Edit" class="ajaxify">
							<button class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></button>
						</a> 
						
                       	<a data-href="'.BASEPATH.'/penerimaan/pembatalanplpasal/delete/'.$entry['ID_RESPONPLP'].'" rel="tooltip" title="Delete" class="delete">
							<button class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></button> 
						</a>';
	   		
			$row[]	= array('<input type="checkbox" name="UID[]" id="UID" value="'.$entry['ID_RESPONPLP'].'" class="checkboxes">',
							$entry['ID_RESPONPLP'],
							'<a data-href="'.BASEPATH.'/penerimaan/pembatalanplpasal/detail/'.$entry['ID_RESPONPLP'].'" rel="tooltip" title="Detail" class="ajaxify">'.$entry['KD_KANTOR'].'</a>',
							$entry['KD_TPS'],
							$entry['REF_NUMBER'],
							$entry['NO_BATAL_PLP'],
							$entry['TGL_BATAL_PLP'],
	...
	{
		$aColumns = array(	'ID_SERVICE_TYPE'	=> 123,
						  	'KD_KANTOR'			=> $data['KD_KANTOR'],
							'KD_TPS'			=> $data['KD_TPS'],
							'REF_NUMBER'		=> $data['REF_NUMBER'],
							'NO_BATAL_PLP'		=> $data['NO_BATAL_PLP'],
